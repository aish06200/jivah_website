...
const HOME_PROJECTS = [
  {
    ...
    crop: "absolute h-[131.53%] w-[99.96%] max-w-none left-[-0.01%] top-[-31.45%]",
  },
  {
  ...
  return (
    <>
      ...
      <section id="projects" className="bg-white py-16 md:py-[120px]">
        ...
        <div className="site-pad mt-16 flex flex-col gap-10 md:flex-row md:gap-[50px] md:overflow-x-auto md:pb-2 md:[scrollbar-width:none] md:[&::-webkit-scrollbar]:hidden">
          {cards.map((project) => (
            <Link
              key={project.slug}
              href={`/projects/${project.slug}`}
              aria-label={`${project.name}, ${project.startingPrice}, ${project.city}`}
              className="group flex w-full flex-col gap-4 md:w-[min(80vw,615px)] md:shrink-0"
            >
              <div className="relative h-[360px] overflow-hidden rounded-[8px] bg-paper md:h-[538px]">
                <img
                  src={withBase(project.image)}
                  alt=""
                  className={`${project.crop} transition-transform duration-700 group-hover:scale-[1.04]`}
                />
                ...
                <div className="absolute inset-x-0 bottom-0 flex flex-col gap-4 p-5 md:p-6">
                  ...
                  >
                    {project.startingPrice === "Sold out"
                      ? "Sold out"
